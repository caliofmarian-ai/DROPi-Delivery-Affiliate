...
import { getArticle } from './src/content.mjs';
import { homePage, guidesPage, articlePage, toolsPage, aboutPage, disclosurePage, privacyPage, outboundUrl, sitemap, siteUrl, layout } from './src/site.mjs';
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (url.pathname === '/health') return send(res, 200, JSON.stringify({ ok: true }), 'application/json; charset=utf-8', { 'cache-control': 'no-store' });
